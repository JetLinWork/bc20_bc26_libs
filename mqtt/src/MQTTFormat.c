...
int MQTTStringFormat_connect(char* strbuf, int strbuflen, MQTTPacket_connectData* data)
{
	...
	strindex = Ql_strstr(strbuf, strbuflen,
			"CONNECT MQTT version %d, client id %.*s, clean session %d, keep alive %d",
			(int)data->MQTTVersion, data->clientID.lenstring.len, data->clientID.lenstring.data,
			(int)data->cleansession, data->keepAliveInterval);
	if (data->willFlag)
		strindex += Ql_strstr(&strbuf[strindex], strbuflen - strindex,
...
{
	int strindex = Ql_strstr(strbuf, strbuflen, "CONNACK session present %d, rc %d", sessionPresent, connack_rc);
	return strindex;
}
...
{
	int strindex = Ql_strstr(strbuf, strbuflen,
...
{
	int strindex = Ql_strstr(strbuf, strbuflen, "%s, packet id %d", MQTTPacket_names[packettype], packetid);
	if (dup)
		strindex += Ql_strstr(strbuf + strindex, strbuflen - strindex, ", dup %d", dup);
	return strindex;
}
...
{
	return Ql_strstr(strbuf, strbuflen,
...
char* MQTTFormat_toClientString(char* strbuf, int strbuflen, unsigned char* buf, int buflen)
{
	...
	case PINGRESP:
	case DISCONNECT:
		strindex = Ql_strstr(strbuf, strbuflen, "%s", MQTTPacket_names[header.bits.type]);
		break;
	}
	return strbuf;
}
